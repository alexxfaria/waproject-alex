...
const SaleListPage = memo(() => {
  const [formOpened, setFormOpened] = useState(false);
  const [current, setCurrent] = useState<ISale>();

  const [params, mergeParams, loading, data, error, , refresh] = usePaginationObservable(
    params => saleService.list(params),
  ...
  const handleEdit = useCallback((current: ISale) => {
    setCurrent(current);
    setFormOpened(true);
  }, []);
  ...
  const formCallback = useCallback(
    (sale?: ISale) => {
      setFormOpened(false);
      current ? refresh() : mergeParams({ term: sale.description });
    },
  ...
  return (
    <Fragment>
      ...
      <Card>
        <FormDialog opened={formOpened} sale={current} onComplete={formCallback} onCancel={formCancel} />
        ...
        <TableWrapper minWidth={500}>
          <Table>
            ...
            <TableBody>
              ...
              />
              {results.map(sale => (
                <ListItem key={sale.id} sale={sale} onEdit={handleEdit} onDeleteComplete={refresh} />
              ))}
            </TableBody>
          </Table>
        </TableWrapper>
